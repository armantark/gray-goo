...
The approved prototype uses Quark Dust Ladder, Coral Colony Tide Pool, Skatepark Bowl, and Tablecloth of Everything to explore a representative range of scales. The user is comfortable with cartoon subatomic representations and expects larger scales to provide richer interactions. The target duration is roughly 3–5 minutes per typical level.

The full campaign culminates at cosmic scales and consuming spacetime. Puddle consumption is local and progressive: each contacted section disappears into the goo, leaving the rest of the translucent liquid behind. The goo rolls and flows across the floor as one continuous body, with sticky contacts that stretch and peel. Uneven idle oozing, squashing, stretching, sticky contact, and springy eyes are central to the intended feel.
...
Guide eating with a pointer toward the nearest edible object and a highlight on that same target. Show the last consumed item as a named portrait in the bottom-left corner. Consumed food first leaves a localized patch of its average color where it touches the goo. That patch gradually blends through the organism to leave a lasting tint, rather than flashing back to gray.
