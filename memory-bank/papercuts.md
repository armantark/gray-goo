...
2026-09-22 17:41 ticket 07: a native capture that saved each PNG mid-series (Image.save_png at 1920x1080) blocked about 0.3 s per frame, and the physics catch-up after each save showed as 37.55 and 42.0 px/frame² camera spikes; scripts/capture_jump_camera.gd now holds images in memory and writes them after each series.
...
- 2026-09-23T01:12:33Z ticket 04: freeing the goo to reposition it stops every meal in flight (the meal animation needs a valid `_meal_target`), leaving eaten food visible; rebind `_meal_target` to the new body as `_switch_body` does, or wait 0.34 s, before judging leftovers.
- 2026-09-23T01:12:33Z ticket 04: a probe run against stashed (unfixed) source called a helper the fix adds (`Food._shows`) and logged `Nonexistent function '_shows'`; before-fix probes must use only APIs that exist on both sides.
- 2026-09-22 17:40 PDT: collision ticket 03: a probe that called `game._physics_process` and `goo._physics_process` 240 times inside one `_run` never let Jolt step, so a loose can reported `can_moved/r=0.0`; drive probes with `await physics_frame` when food physics matters.
- 2026-09-22 17:58 PDT: collision ticket 03: `--write-movie` with `--resolution 960x540` still wrote 1920x1080 PNGs, and ImageMagick `montage` prints `unable to read font` on every call (images are still written); crop with `magick 'f.png[WxH+X+Y]'` before montage.
- 2026-09-22 18:20 PDT: collision ticket 03: the route driver's body and results vary run to run (the shell gait calls `randomize()`), so a single Skatepark run said 0 stalls over 2 s and the next said 1 (3.95 s); judge stall claims over several runs.
- 2026-09-22 18:38 PDT: collision ticket 03: a zsh loop `for spec in "shell 2.0"; do set -- $spec` does not word-split, so three route runs got `--speed=` (parsed as 0, the goo never moved) and `--body=procedural 1.0` silently; wasted about 10 minutes. The driver now rejects a non-positive speed or unknown body; wrap such loops in `bash -c`.
- 2026-09-22 21:00 PDT ticket 12: `Godot --headless` never fires `RenderingServer.frame_post_draw`, so a headless capture script hangs with no image (a 60 s timeout printed nothing). Native screenshots need a short window.
- 2026-09-22 20:59 PDT ticket 12: Blender Freestyle `line_thickness` in ABSOLUTE mode is multiplied by the line style's own thickness (default 3), so a 2 px setting drew about 6 px lines and turned a 50 px game-size render into a dark blob.
- 2026-09-22 21:01 PDT ticket 12: per-face material panels inside one mesh bake as a pixel staircase in the 512 px atlas, because the border falls inside one smart-project UV island. Separate the panels by material after subdivision.
- 2026-09-22 21:27 PDT ticket 12: setting `game.goo.global_position` in a capture probe does not move the goo. Frame the camera on the target with `rig.configure(..., target, ...)`, or rebuild the body the way `capture_composites.gd` `_place_goo` does.
- 2026-09-22 21:24 PDT ticket 12: GPT-6 Astra's model scores moved by about 0.2 on unchanged dimensions between passes, and its waist advice reversed from one pass to the next. The rule of three passes without a gain can stop on critic noise alone.
