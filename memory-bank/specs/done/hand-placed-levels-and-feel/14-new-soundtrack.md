...
- [x] Each piece has a LilyPond source in the repository, an engraved score image that Opus looked at, and a rendered Ogg that the game plays.
- [x] The loop point of each Ogg has no audible click or gap: the last and first 50 milliseconds match in level, and a spectrogram image across the loop shows no break.
- [x] Each Ogg is under the browser build's 25 MiB per-file limit, and the loudness of the four pieces is within 2 LU of each other.
- [x] The old numpy synthesis script and its score file are deleted, and `memory-bank/techContext.md` describes the new build command.
