...
- [x] The cause of the fast Level 1 finish is named in `memory-bank/progress.md` with the numbers that show it.
- [ ] The route driver finishes Sugar Water in 6 to 10 minutes at 100%. Not met: the route wins in 140.7 to 160.3 s. Its atom and molecule views hold too few objects for longer views at eight body lengths per second (see `memory-bank/progress.md`), so this moves to ticket 09's spawn points and placed layout.
- [x] The settings menu shows 100% as the default, with no truncation.
