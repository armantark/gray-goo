...
## Current state, 2026-09-22 19:55 (hand-placed levels and game feel)

Spec and tickets: `memory-bank/specs/hand-placed-levels-and-feel/` (spec.md, tickets 01–14). Plan page: `memory-bank/status-updates/hand-placed-levels-plan-2026-09-22.html`, hosted at https://b26t4r49k9fu.postplan.dev (keep unchanged). Opus 5.5 writes all code; the orchestrator dispatches one Opus 5.5 agent per ticket, parallel tickets in treehouse worktrees on their own branches, then merges into master with `--no-ff` after body/obstacle/world checks pass.

Done and merged on master (head 1b4dd3a before ticket 08): tickets 01 (route driver measures missed meals and stalls), 02 (Tasty Planet eat rule: footprint < goo radius × 1.2, tier ignored, first touch eats; food grid refreshes moving food), 03 (goo slides around what it cannot eat), 04 (emptied wholes never linger; nucleus and hermit crab sizes), 05 (100% = old 200%, 8 body lengths/s; emptied wholes pay no growth; all four levels winnable), 06 (meal feedback scaled by reward, edible signal, reward-sized arrow, all-caps HUD labels removed), 07 (size-jump slow moment, eased zoom, tier card, look-ahead camera), 14 (Muse Sounds jazz-band soundtrack plus Larry Seyer SFZ upright bass via `scripts/sfz_player.py`). Commit 7937c70 removed whole-model recolors on skaters, boards, rails, crab, and galaxies (owner disliked skaters turning brown); coral bleaching and element colors keep their recolor.

In progress: ticket 08 (spawn points, placement table API, top-down capture `scripts/capture_layout.gd`, hand-placed Tide Pool) by an Opus 5.5 agent in the main checkout; uncommitted work at this time: scripts/check_worlds.gd, src/levels/tide_pool.gd, src/pool.gd, src/world.gd, scripts/capture_layout.gd. On resume, check `git status` and the ticket 08 status line; if the agent died from quota, dispatch a fresh ticket 08 agent that continues from the working tree.

Next: after 08, run 09 (Sugar Water), 10 (Skatepark Bowl), 11 (Cosmic Web), and 12 (3D model test) in parallel worktrees; then 13 (release check, results artifact, move spec to specs/done/).

Inputs for tickets 09–11: level length is still short (Sugar Water 140.7 s, Skatepark 72.8 s, Cosmic Web 300.1 s at 100%; target 6–10 min) because the upper views lack food, so spawn points and density must supply it. Sizes that do not match drawings: skater 0.9 draws 0.628; pion 0.21 draws 0.303; early nucleons 0.18 vs about 0.305; hermit crab shell 0.475 draws 0.635; rim boulder with anemone 2.496 draws 3.037. Skatepark cone at (1, 28) and shoe and water bottle near (40, -14) sit inside quarter-pipe outlines. Sucrose and the supercluster are marked milestones but cannot be eaten; nothing reads the milestone flag. The procedural body's contact test aims above center, so anemones may not react.

Standing rules from the owner: all route runs headless with `--audio-driver Dummy` and the simulation clock; native windows only for short muted captures; no Gemini listening (the owner judges music). Final music WAVs for the owner: `builds/music/final-*.wav`.

Owner actions and open items: set the speed slider to 100% once (saved 200% now means 4× old speed). Before any public release, check that the free Muse Sounds license and the Larry Seyer pack ("(c) 1995 Larry Seyer All Rights Reserved", "may be freely distributed") allow use in a published game. The Blender MCP server is registered for Claude at local scope (see techContext.md); it loads in a new session and needs Blender open through `scripts/blender/open_session.sh`. Ticket 12 ran without it (headless Blender renders), shipped the new skate shoe at a 7.08 plateau, and is merged. The "last eaten" HUD card shows a plain oval for a skateboard; add to the release check. The memory-bank staleness sweep is overdue and not approved yet.
...
Publication is complete. Source and Mac download: https://github.com/armantark/gray-goo/releases/latest . Public browser build (no login): https://gray-goo.tarkavor.chatgpt.site . Results: https://hyio886n4pf3.postplan.dev . The user requested public access, which is enabled. Anonymous checks exposed missing compression headers; the web preloader now decodes gzip directly so ordinary static hosting works. The public deployment passed anonymous HTTP checks: game files match the local export, and Sites adds only its Cloudflare script to the application HTML. Local browser play and all native music loops passed. Exact release and deployment evidence is in progress.md. The project is shelved. Do not create a future iteration or automation without a new user request.
