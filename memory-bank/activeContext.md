...
Owner actions and open items: set the speed slider to 100% once (saved 200% now means 4× old speed). Before any public release, check that the free Muse Sounds license and the Larry Seyer pack ("(c) 1995 Larry Seyer All Rights Reserved", "may be freely distributed") allow use in a published game. The Blender MCP server is configured for Claude in `.mcp.json` with only three local tools allowed; it loads in a new session and needs Blender running with the add-on started (`scripts/blender/start_session.py`). The "last eaten" HUD card shows a plain oval for a skateboard; add to the release check. The memory-bank staleness sweep is overdue and not approved yet.
