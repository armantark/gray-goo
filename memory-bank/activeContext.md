...
Food guidance: use a pointer toward the nearest edible object, following the user's stated preference for the original game's guidance rather than highlighting nearby food. The goo should take on the average color of consumed food with a lasting impression, not a brief flash back to gray. Color mixing and persistence are still open. The user clarified that coral consists of distinct composite parts; do not interpret it as a continuously eroded lump or assume that removing parts automatically recalculates the whole's eating threshold.
