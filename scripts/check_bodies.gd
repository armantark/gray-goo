...
func _run() -> void:
	...
	_check(game.goo.radius > initial_radius, "body grows")
	_check(game.goo.tint.is_equal_approx(deposited_tint), "pigment survives diffusion")
	_check(not meal.visible and meal._meal_age >= 0.34, "meal animation finishes")
	var start: Vector3 = game.goo.global_position
	Input.action_press("move_up")
	for tick in 60:
		_tick(game)
	Input.action_release("move_up")
	_check(game.goo.global_position.distance_to(start) > initial_radius, "game input moves body")
	...
	_check(game.goo.global_position.x > edge_start.x, "wall probe drives outward")
	_check(grounded, "skin stays above terrain")
	_check(bounded, "skin stays inside field")
	_check_eat_rule(game)
	print("BODY_CASE radius=", game.goo.radius, " position=", game.goo.global_position)
...
func _check_eat_rule(game: Node3D) -> void:
	game.start_level(1)
	...
	game._physics_process(1.0 / 60.0)
	_check(not small.active, "smaller later-tier object is eaten on contact")
	...
	_check(touched and large.active and solid, "larger object blocks")
	_check(clear, "goo stays out of the larger object's footprint")
	_check((game.goo.global_position - large.global_position).dot(heading) > 0.0, "goo slides past the larger object")
