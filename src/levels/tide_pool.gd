...
const CROWN_POSITION := Vector2(10, -23)
...
func definition() -> Dictionary:
	return {"title": "Coral Colony Tide Pool", "meters_per_unit": 0.01,
		...
		"start_position": Vector3(-24.0, 0.0, 17.0), "field": Rect2(-48, -43, 96, 86),
		...
		"jumps": [{"radius": 0.55, "view_size": 14.0}, {"radius": 0.85, "view_size": 20.0},
			{"radius": 1.35, "view_size": 28.0}, {"radius": 2.1, "view_size": 38.0},
			{"radius": 3.0, "view_size": 52.0}]}

func ground_height(point: Vector3) -> float:
	var radial := Vector2(point.x / 39.0, point.z / 34.0).length()
	var angle := atan2(point.z / 34.0, point.x / 39.0)
	var shore := 1.0 + 0.055 * sin(angle * 3.0 + 0.4) + 0.03 * sin(angle * 5.0 - 1.1)
	var rim := smoothstep(0.75, 1.04, radial / shore)
	var deep_end := exp(-Vector2((point.x - 10.0) / 19.0, (point.z + 20.0) / 15.0).length_squared())
	return lerpf(-0.42 - deep_end * 1.15, 1.15, rim)

func build(world: GameWorld) -> void:
	_world = world
	_build_rim()
	_water = world._pool(Vector3(0, -0.12, 0), Vector2(39, 34), Color(0.16, 0.69, 0.75, 0.36), 8.0)
	_water.name = "The basin water"
	_water.min_tier = 3
	_water.minimum_radius = 2.1
	_water.set_basin(ground_height)
	_build_coral()
	_build_current()
	_build_animals()
	var crown := _world._add_food("coral_fan", CROWN_POSITION, 4.6, 3.0, 30.0, "Great coral crown", false, 4)
...
func _build_rim() -> void:
	...
	for index in range(28):
		var angle := (index + _world._rng.randf_range(-0.34, 0.34)) * TAU / 28.0
		var shore := 1.0 + 0.055 * sin(angle * 3.0 + 0.4) + 0.03 * sin(angle * 5.0 - 1.1)
		var at := Vector2(cos(angle) * 36.0, sin(angle) * 31.0) * (shore + _world._rng.randf_range(-0.035, 0.035))
		var rock := _world._add_food("boulder", at, _world._rng.randf_range(1.55, 2.5), 2.1, 0.48, "Pool rim boulder", false, 3)
		...
		var between := Vector2(cos(angle + 0.07) * 40, sin(angle + 0.07) * 35) * shore
...
func _build_coral() -> void:
	...
	for index in range(shelf.size()):
		...
		var at: Vector2 = shelf[index] + Vector2(_world._rng.randf_range(-0.7, 0.7), _world._rng.randf_range(-0.7, 0.7))
		var head := _world._add_food("", at, 2.0, 1.35, 0.24, "Coral head", false, 2)
		...
		var snail := _world._add_food(snail_kind, at + Vector2.from_angle(angle) * _world._rng.randf_range(2.6, 3.4), 0.44, 0.85, 0.035, snail_kind.capitalize(), false, 1)
...
func _add_branch(head: Food, at: Vector2) -> void:
	...
	var branch := _world._add_food("coral_branch", at, size, 0.85, 0.035, "Living coral branch", false, 1, head)
	...
	for tip in [Vector3(-0.61, 0.40, 0.01), Vector3(-0.43, 0.48, -0.04),
		...
		_world._add_food("polyp", Vector2(tip.x, tip.z) * size / 0.72, 0.15, 0.34, 0.003, "Living polyp", false, 0, branch, tip.y * size / 0.72)
...
func _build_current() -> void:
	...
	for index in range(48):
		...
		var food := _world._add_food("plankton", Vector2(at.x, at.z), _world._rng.randf_range(0.15, 0.22), 0.32, 0.0025, "Current plankton")
		food.position = at
...
func _current_position(item: Dictionary) -> Vector3:
	...
	var at := Vector3(point.x, 0, point.y)
...
func _build_animals() -> void:
	var refuges := [Vector2(-18, 15), Vector2(-28, -2), Vector2(-11, -15),
		Vector2(7, 12), Vector2(25, 0), Vector2(23, -13)]
	for refuge in refuges:
		var angle := _world._rng.randf_range(-PI, PI)
		var at: Vector2 = refuge + Vector2(_world._rng.randf_range(-1.2, 1.2), _world._rng.randf_range(-1.2, 1.2))
		var crab := _world._add_food("", at, 0.9, 1.35, 0.36, "Hermit crab", false, 2)
		...
		_world._add_food("shell", Vector2(0.0, 0.08), 0.54, 0.85, 0.05, "Hermit crab shell", false, 1, crab, 0.14)
		...
		var star := _world._add_food("sea_star", at + Vector2(2.8, 2.4), 0.85, 1.35, 0.3, "Sea star", false, 2)
...
func _add_anemone(parent: Food, at: Vector2) -> void:
	var anemone := _world._add_food("anemone", at, 0.66, 1.35, 0.24, "Sea anemone", false, 2, parent)
	if parent == null:
...
func _build_fish_refuge() -> void:
	var shelter := _world._add_food("", Vector2(-10, -8), 2.5, 2.1, 0.7, "Fish shelter rock", false, 3)
	...
	roof.scale.y *= 0.3
	for side in [-1.0, 1.0]:
		_world._add_food("rock", Vector2(side * 1.75, 0), 0.6, 2.1, 0.24, "Shelter rock foot", false, 3, shelter)
	var fish := _world._add_food("small_fish", Vector2(-10, -8), 0.68, 1.35, 0.42, "Small pool fish", false, 2, null, 0.16)
...
func _build_beach() -> void:
	for edge in [Rect2(-76, -76, 152, 33), Rect2(-76, 43, 152, 33),
		...
		slab.size = Vector3(edge.size.x, 0.45, edge.size.y)
		beach.mesh = slab
		beach.material_override = _sand_material(Color("e4d3a2"))
		beach.position = Vector3(edge.get_center().x, -0.6, edge.get_center().y)
	...
	for at in [Vector2(-61, -13), Vector2(56, 31)]:
		...
		water.position = Vector3(at.x, -0.23, at.y)
