...
# While driven, floor grips travel with most of the body's motion: grips fixed to the ground would
# hold the shell to about half its commanded speed and stretch its tail to several widths long.
const ANCHOR_FOLLOW := 0.85
...
func _step(dt: float) -> void:
	...
	for i in _points.size():
		...
		if _attached[i]:
			...
			_anchors[i] += Vector3(desired.x, 0.0, desired.z) * dt * ANCHOR_FOLLOW * float(moving)
